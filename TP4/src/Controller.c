	...
	int tamNacionalidades,int* idJugadores,ePosicion listaPosi[],int tamPosi)
{
   ...
   if(pArrayListJugador!=NULL)
   {
			...
			utn_pedirInt(&idNacionalidad,"\n>Ingrese ID de nacionalidad: ","\n[!]Error, reintente.",1,32,3)==0)
	   {
		   ...
		   jugador = jug_newParametros(*idJugadores,nombre,edad,posicion,nacionalidad,"0");
		   tam=ll_len(pArrayListJugador);
		   system("CLS");
		   if(utn_pedirInt(&opcion,"\n1.Aniadir jugador al final de la lista(ll_add)."
				   "\n2.Colocar y reemplazar en el indice(ll_set)."
				   "\n3.Colocar y desplazar en el indice (ll_push).",
				   "\nError, reintente.",1,3,3)==0 && jugador!=NULL)
		   {
			   switch(opcion)
			   {
			   case 1:
				   if(ll_add(pArrayListJugador,jugador)==0)
				   {
					   *idJugadores+=1;
					   	retorno = 0;
				   }
				   break;
			   case 2:
				   if(utn_pedirInt(&opcion,"\nIngrese el indice donde colocar el elemento: ",
						   "\nError, reintente",0,tam,3)==0 &&
						   ll_set(pArrayListJugador,opcion,jugador)==0)
				   {
					   *idJugadores+=1;
					   	retorno = 0;
				   }
			   ...
			   case 3:
				   if(utn_pedirInt(&opcion,"\nIngrese el indice donde colocar el elemento: ",
				   		"\nError, reintente",0,tam,3)==0 &&
				   		ll_push(pArrayListJugador,opcion,jugador)==0)
				   	{
					   *idJugadores+=1;
				   		retorno = 0;
				    }
	...
	do{
		...
		if(utn_pedirInt(&opcion,"\n>Ingrese opcion: ","\n[!]Error, reingrese.",1,5,3)==0)
		{
			...
			case 3:
				...
				break;
			case 4:
				if(ll_isEmpty(listaClonada)==0){
				controller_listarJugadores(listaClonada,listaSelecciones);
				printf("\n");
				}else{
					printf("\nLa lista clonada no tiene elementos o no existe.\n");
				}
				system("PAUSE");
				break;
			case 5:
